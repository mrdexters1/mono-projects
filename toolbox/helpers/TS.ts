 ...
 * ```
 */
export type AsyncReturn<Fn extends AnyFn> = Fn extends AnyFn<infer T> ? Awaited<T> : never;
 ...
 * ```
 */
export type PartialByKey<T, K extends keyof T = keyof T> = Omit<T, K> & Partial<Pick<T, K>>;
 ...
 * ```
 */
export type PartialExceptKey<T, K extends keyof T> = Partial<Omit<T, K>> & Required<Pick<T, K>>;
 ...
 * ```
 */
export type AssertsWith<T> = (condition: unknown, thing: T) => asserts condition;
